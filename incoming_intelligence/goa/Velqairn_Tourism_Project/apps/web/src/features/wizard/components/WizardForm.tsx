    ...
    if (data.daily_island_plan && data.number_of_days) {
      ...
      data.daily_island_plan = plan.map((day: any, idx: number) => {
        const resolvePrimaryIsland = (candidate: string, attractions: string[]) => {
          ...
          const northGoa = ["north goa", "candolim", "calangute", "baga", "anjuna", "vagator", "morjim", "mandrem", "arambol", "chapora", "aguada", "ashwem", "siolim", "assagao", "tiracol"];
          const southGoa = ["south goa", "colva", "benaulim", "varca", "cavelossim", "mobor", "agonda", "palolem", "patnem", "cabo de rama", "netravali", "majorda", "utorda", "betalbatim", "canacona", "bogmalo", "vasco"];
          if (northGoa.some(k => text.includes(k))) return "North Goa";
          if (southGoa.some(k => text.includes(k))) return "South Goa";
          if (["old goa", "basilica", "se cathedral", "divar", "chorao"].some(k => text.includes(k))) return "Old Goa";
          if (["dudhsagar", "collem", "mollem", "spice plantation"].some(k => text.includes(k))) return "Dudhsagar";
          if (["panaji", "panjim", "fontainhas", "miramar", "dona paula", "mandovi", "waterfront", "museum"].some(k => text.includes(k))) return "Panaji";
          if (text.includes("departure")) return "Departure";
          return idx === 0 ? "Panaji" : (candidate && candidate.toLowerCase() !== "goa" ? candidate : "North Goa");
        };
