...
from app.services.goa_geography import normalize_goa_region
...
- "travel_movement": Set strictly to the canonical movement:
  * Day 1: "Panaji" (or arrival base)
  * Regional transfer: "Origin to Destination" (e.g. "Panaji to North Goa", "North Goa to South Goa", "South Goa to Panaji")
  * Day-trip excursion returning to base hotel: "Origin to Destination Return" (e.g. "Panaji to Dudhsagar Return")
  * Exploration within the same region: "Region Name" (e.g. "North Goa")
...
1. "END OF THE JOURNEY" (contains TWO distinct paragraphs):
   - PARAGRAPH 1 (departure_narrative): Concludes the journey with comfortable hotel check-out, luggage assistance, and private transfer from the hotel to Goa airport, ensuring a smooth and hassle-free journey home (2–3 sentences, ~35-50 words).
     Example: "Enjoy a peaceful morning check-out at [Hotel] with full luggage assistance. Your private chauffeur will pick you up for a smooth transfer to Goa airport for your flight home."
   - PARAGRAPH 2 (farewell_narrative): Sincere gratitude from Darun Tourism for choosing us, pleasure in crafting memories, safe flight wishes, and welcoming them back in the future (2–4 sentences, ~40-60 words).
     Example: "Darun Tourism extends its heartfelt gratitude to [Customer Name] and family for choosing us. It was our genuine pleasure crafting your Goa trip memories, and we look forward to welcoming you back in the future."
...
def _resolve_primary_island(island_candidate: str, attractions: list[str], day_num: int, total_days: int) -> str:
    ...
    if any(a.strip().lower() == "departure" for a in attractions or []):
        return "Departure"
    return normalize_goa_region(text) or "North Goa"
...
def _format_day_by_day_database_directives(request: TripRequest) -> str:
    ...
    for day_num in range(1, total_days + 1):
        ...
        attractions = ", ".join(dp.attractions) if dp and dp.attractions else "Key Goa highlights"
