...
def highlight_sections(itinerary_text: str, request: TripRequest) -> list[dict[str, str]]:
    ...
    dest_full = format_list(data["destinations"], request.destination or "Andaman Islands")
    dest_short = format_list_short(data["destinations"], 45, request.destination or "Andaman Islands")
